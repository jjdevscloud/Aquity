...
contract LauncherTest is Test {
    ...
    function setUp() public {
        ...
        launcher = new Launcher(admin, address(registry), address(factory));
    ...
    function test_launch_registersMintsAndLocksVesting() public {
        ...
        vm.prank(builder);
        (uint256 tokenId, address agentToken, address pool, address vestingAddr) = launcher.launch{value: 1 ether}(p);

        assertEq(registry.ownerOf(tokenId), builder);
        assertEq(pool, address(factory));
